...
type AdoptionStatus = 'AVAILABLE' | 'RESERVED' | 'ADOPTED' | 'EXPIRED';
...
export async function GET(request: NextRequest): Promise<NextResponse<AdoptionListResponse>> {
  try {
    ...
    const apiUrl = new URL(`${process.env.API_SERVER_URL}/api/adoption/list`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        ...
        message: errorData.message || '입양 공고 목록 조회 중 오류가 발생했습니다.',
        result: null
      }, { status: response.status });
    }

    const result: AdoptionListResponse = await response.json();
    return NextResponse.json(result);

  } catch (error) {
    console.error('Adoption list fetch error:', error);
