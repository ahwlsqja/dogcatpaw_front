...
interface StoryDailySearchResponse {
  isSuccess: boolean;
  status: string;
  code: string;
  message: string;
...
export async function GET(request: NextRequest): Promise<NextResponse<StoryDailySearchResponse>> {
  try {
    ...
    const apiUrl = new URL(`${process.env.API_SERVER_URL}/api/story/daily/search`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        ...
        message: errorData.message || '일지 검색 중 오류가 발생했습니다.',
        result: null
      }, { status: response.status });
    }

    const result: StoryDailySearchResponse = await response.json();
    return NextResponse.json(result);

  } catch (error) {
    console.error('Story daily search error:', error);
