...
interface StoryDailyListResponse {
  isSuccess: boolean;
  status: string;
  code: string;
  message: string;
...
export async function GET(request: NextRequest): Promise<NextResponse<StoryDailyListResponse>> {
  try {
    ...
    const apiUrl = new URL(`${process.env.API_SERVER_URL}/api/story/daily/stories`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        ...
        message: errorData.message || '일지 목록 조회 중 오류가 발생했습니다.',
        result: null
      }, { status: response.status });
    }

    const result: StoryDailyListResponse = await response.json();
    return NextResponse.json(result);

  } catch (error) {
    console.error('Story daily list fetch error:', error);
