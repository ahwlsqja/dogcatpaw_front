...
interface StoryReviewListResponse {
  isSuccess: boolean;
  status: string;
  code: string;
  message: string;
...
export async function GET(request: NextRequest): Promise<NextResponse<StoryReviewListResponse>> {
  try {
    ...
    const apiUrl = new URL(`${process.env.API_SERVER_URL}/api/story/review/reviews`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        ...
        message: errorData.message || '후기 목록 조회 중 오류가 발생했습니다.',
        result: null
      }, { status: response.status });
    }

    const result: StoryReviewListResponse = await response.json();
    return NextResponse.json(result);

  } catch (error) {
    console.error('Story review list fetch error:', error);
